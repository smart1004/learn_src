...
w3 = tf.Variable(tf.truncated_normal([0, 0], stddev=0.1))
b3 = tf.Variable(tf.ones([0]))

w4 = tf.Variable(tf.truncated_normal([0, 0], stddev=0.1))
b4 = tf.Variable(tf.ones([0]))

w5 = tf.Variable(tf.truncated_normal([0, 0], stddev=0.1))
b5 = tf.Variable(tf.ones([0]))
...
l1_pool = tf.nn.avg_pool(l1_actv, [0, 0, 0, 0], [0, 0, 0, 0], padding='')

l2_conv = tf.nn.conv2d(l1_pool, w2, [0, 0, 0, 0], padding='')
l2_actv = tf.sigmoid(l2_conv + b2)
l2_pool = tf.nn.avg_pool(l2_actv, [0, 0, 0, 0], [0, 0, 0, 0], padding='')

# [문제 3] 컨볼루션에서 FC로 넘어가는 아래 숫자를 채우세요.
fc_layer = tf.reshape(l2_pool, [-1, 0])
